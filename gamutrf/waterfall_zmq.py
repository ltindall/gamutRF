...
def process_df(df): 
    ...
    df["freq"] = (df["freq"] / scan_fres_resolution).round() * scan_fres_resolution 
    ...
    if meandiff > mindiff * 2:
        print("Mean frequency diff larger than minimum: increase scanner sample rate")
    
    return df

def update_db_limits(fig, cbar_ax, scalar_mappable, db_min, db_max):
    cbar_ax.cla()
    scalar_mappable.set_clim(vmin=db_min, vmax=db_max)
...
def main():
    ...
    while True: 
        ...
        if scan_df is not None: 
            scan_df = process_df(scan_df)
            scan_df.freq = scan_df.freq / scale
            ...
            if counter % draw_rate == 0:
                ...
                #update_db_limits(fig, cbar_ax, sm, db_min, db_max)
